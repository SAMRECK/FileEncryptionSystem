...
static struct block prepare_key(unsigned char s[16]);
static void generate_round_keys(struct block key, struct block round_key[11]);
static struct block add_round_key(struct block data, struct block key);
static struct block next_key(struct block prev_key, int round);

static struct block encrypt(struct block data, struct block round_key[11]);
static struct block decrypt(struct block cipher, struct block round_key[11]);

static struct block sub_byte(struct block data, unsigned char *box);
static struct block shift_rows(struct block data);
...
int aes_encrypt_file(FILE *fp_input, FILE *fp_output, char *key_s)
{
	struct block data;
	struct block key = prepare_key(key_s);
	struct block round_key[11];
	generate_round_keys(key,round_key);	
	unsigned long size=file_size(fp_input);
	int a,b;
	...
	{	for(j=0;j<4;j++)
		{	for(k=0;k<4;k++)
			{	fread(&data.b[j][k],sizeof(data.b[j][k]),1,fp_input);
			}
		}
		data=encrypt(data,round_key);
		for(j=0;j<4;j++)
		{	for(k=0;k<4;k++)
			{	fwrite(&data.b[j][k],sizeof(data.b[j][k]),1,fp_output);
			}
		}
	}
	int c=b;		
...
int aes_decrypt_file(FILE *fp_input, FILE *fp_output, char *key_s)
{
	struct block data;
	struct block key = prepare_key(key_s);
	struct block round_key[11];
